package com.liucj.lib_common.view.flexbox;
...
import com.liucj.lib_common.view.flexbox.logic.CommonLogic;
import com.liucj.lib_common.view.flexbox.logic.ConfigDefinition;
import com.liucj.lib_common.view.flexbox.logic.LineDefinition;
import com.liucj.lib_common.view.flexbox.logic.ViewDefinition;
...
public class FlexBoxLayout extends ViewGroup {
    ...
    public FlexBoxLayout(Context context, AttributeSet attributeSet, int defStyle) {
    ...
    private void readStyleParameters(Context context, AttributeSet attributeSet) {
        TypedArray a = context.obtainStyledAttributes(attributeSet, R.styleable.FlexBoxLayout);
        try {
            this.config.setOrientation(a.getInteger(R.styleable.FlexBoxLayout_android_orientation, CommonLogic.HORIZONTAL));
            this.config.setMaxLines(a.getInteger(R.styleable.FlexBoxLayout_maxLines, 0));
            this.config.setDebugDraw(a.getBoolean(R.styleable.FlexBoxLayout_debugDraw, false));
            this.config.setWeightDefault(a.getFloat(R.styleable.FlexBoxLayout_weightDefault, 0.0f));
            this.config.setGravity(a.getInteger(R.styleable.FlexBoxLayout_android_gravity, Gravity.CENTER_VERTICAL));
            this.drawBorder = a.getBoolean(R.styleable.FlexBoxLayout_drawBorder, false);

            int layoutDirection;
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
                layoutDirection = a.getInteger(R.styleable.FlexBoxLayout_layoutDirection, LAYOUT_DIRECTION_LTR);
            } else {
                layoutDirection = a.getInteger(R.styleable.FlexBoxLayout_layoutDirection, super.getLayoutDirection());
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int fixWidth = 0;
        if (lineItemNum != 0) {
            fixWidth = (widthMeasureSpec & 0xFFFF) / lineItemNum;
        }
        views.clear();
        lines.clear();
        for (int i = 0; i < count; i++) {
            ...
            if (lineItemNum != 0) {
                childWidth = childWidth > fixWidth ? childWidth : fixWidth;
            }
            view.setWidth(childWidth);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        borderLinePath.addRect(0, 0, r - l, b - t, Path.Direction.CW);
        final int linesCount = this.lines.size();
        for (int i = 0; i < linesCount; i++) {
            final LineDefinition line = this.lines.get(i);
            borderLinePath.moveTo(0, line.getLineStartThickness() + line.getLineThickness());
            borderLinePath.lineTo(r - l, line.getLineStartThickness() + line.getLineThickness());
            ...
            for (int j = 0; j < count; j++) {
                ...
                int borderLineX = this.getPaddingLeft() + line.getX() + child.getInlineX() + lp.leftMargin + child.getWidth();
                if (r - l - borderLineX > 50) {
                    borderLinePath.moveTo(borderLineX, line.getLineStartThickness());
                    borderLinePath.lineTo(borderLineX, line.getLineStartThickness() + line.getLineThickness());
                }
            }
        }
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (this.drawBorder) {
            borderLinePaint.setAntiAlias(true);//设置画笔为无锯齿
            borderLinePaint.setColor(getResources().getColor(R.color.table_border));//设置画笔颜色
            //canvas.drawColor(Color.WHITE);//白色背景
            borderLinePaint.setStrokeWidth((float) 1.0);//线宽
            borderLinePaint.setStyle(Paint.Style.STROKE);
            //canvas.drawLine(0, 0, 100, 100, borderLinePaint);
            canvas.drawPath(borderLinePath, borderLinePaint);
        }
    ...
    /**
     * @param borderLine 设置绘制表格框
     */
    public void setBorderLine(boolean borderLine) {
        this.drawBorder = borderLine;
    }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        private void readStyleParameters(Context context, AttributeSet attributeSet) {
            TypedArray a = context.obtainStyledAttributes(attributeSet, R.styleable.FlexBoxLayout_LayoutParams);
            try {
                this.newLine = a.getBoolean(R.styleable.FlexBoxLayout_LayoutParams_layout_newLine, false);
                this.gravity = a.getInt(R.styleable.FlexBoxLayout_LayoutParams_android_layout_gravity, Gravity.NO_GRAVITY);
                this.weight = a.getFloat(R.styleable.FlexBoxLayout_LayoutParams_layout_weight, -1.0f);
            } finally {
                a.recycle();
            }
        }
